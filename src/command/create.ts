...
export const checkVersion = async (name: string, version: string) => {
  ...
  if (need) {
    console.warn(
      `检查到heiqi-cli最新版本： ${chalk.blackBright(latestVersion)}，当前版本是：${chalk.blackBright(version)}`
    );
    console.log(
      `可使用： ${chalk.yellow('npm install heiqi-cli@latest')}，或者使用：${chalk.yellow('heiqi update')}更新`
    );
  }
  return need;
};
